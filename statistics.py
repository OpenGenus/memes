...
for formatName in fileSizes:
    if fileSizes[formatName] != 0:
        ...
        xCoord = int(size/2 + (size/4)*math.cos(math.radians((currAngle + (newAngle+currAngle))/2))) - 10#-5 accounts for length of text
        yCoord = int(size/2 + (size/4)*math.sin(math.radians((currAngle + (newAngle+currAngle))/2)))
